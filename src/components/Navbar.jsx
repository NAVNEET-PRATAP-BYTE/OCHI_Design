...
const Navbar = () => {
  ...
  useEffect(() => {
    ...
    // ScrollTrigger for handling scroll direction
  ...
  return (
    <div 
      ref={navRef}
      className="fixed z-[9] w-full px-4 md:px-20 py-4 md:py-8 flex justify-between items-center
    ...
    >
      ...
      <div className="hidden md:flex  gap-10 text-black font-bold ">
        {["Services","Our Work","About Us","Insights","Contact"].map((item,index)=>(
          <a key={index} className={`text-lg capitalize font-bold ${index===4 && "ml-32"}`} href="">{item}</a>
        ))}
      </div>

      {/* Mobile Navigation */}
      <div className={`${isMenuOpen ? 'flex' : 'hidden'} md:hidden absolute top-full left-0 right-0 flex-col items-center  backdrop-blur-md py-4 shadow-lg`}>
        {["Services","Our Work","About Us","Insights","Contact"].map((item,index)=>(
          <a 
            key={index} 
            className="text-lg capitalize  text-black font-bold py-2 hover:bg-gray-100 w-full text-center"
            href=""
          >
            {item}
          </a>
